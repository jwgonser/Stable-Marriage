...
/*fn marry(proposers : Vec<Person>, proposees : Vec<Person>) -> Vec<(String, String)> {

    let mut proposers = proposers;
    let mut proposees = proposees;
    let mut marriage_list = Vec::new();
    let mut complete = false;

    // Decide whether or not all pairing has been completed
    while !complete {
        complete = true;
        let mut unmatched = Vec::new();
        ...
        for p in proposers.iter_mut() {
            ...
            if p.currently_matched == false {
                // iterate through all the proposees
                for i in proposees.iter_mut() {
                    // find proposee
                    if p.preference_list[p.position] == i.name {
                        let mut rank = <i32>::max_value();
                        // set rank to proposee's preference value
                        for x in 0..i.preference_list.len() {
                            if i.preference_list[x] == p.name {
                                rank = x as i32 ;
                                break
                            }
                        }
    ...
    for x in proposers {
        marriage_list.push((x.name, x.preference_list[x.position].clone()));
    }
